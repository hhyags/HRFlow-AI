...
export async function provisionFirebaseUser({
  ...
  if (!user) {
    ...
    } else if (requestedRole !== roles.EMPLOYEE) {
    ...
    if (newOrganization) {
      user = await prisma.$transaction(async (transaction) => {
        const organization = await transaction.organization.create({ data: newOrganization })
        return transaction.user.create({
          data: { ...userData, organizationId: organization.id },
        })
      })
    } else if (acceptedInvitationId) {
      user = await prisma.$transaction(async (transaction) => {
          ...
          return transaction.user.create({ data: userData })
        })
    } else {
  ...
  await writeAuditLog({
    ...
    metadata: {
      ...
      claimsReconciled: claimsChanged,
    },
    prisma,
  })
