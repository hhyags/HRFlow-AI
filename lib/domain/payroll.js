...
export function payslipTemplate(theme = {}) {
  return {
    page: { width: 612, height: 792 },
    colors: {
      primary: theme.primary || [79, 70, 229],
      text: theme.text || [15, 23, 42],
      muted: theme.muted || [100, 116, 139],
      summary: theme.summary || [245, 247, 255],
    },
    spacing: {
      margin: 42,
      headerHeight: 80,
    ...
    labels: {
      product: theme.productName || 'HRFlow AI',
      document: theme.documentLabel || 'PAYSLIP',
      footer: theme.footer || 'This is a system-generated payslip.',
    },
  }
}
...
export async function createPayslipPdf({ organization, payroll, employee, items, currencyCode = 'USD' }) {
  const template = payslipTemplate(organization.payslipTheme || {})
  const pdf = await PDFDocument.create()
  const page = pdf.addPage([template.page.width, template.page.height])
  const regular = await pdf.embedFont(StandardFonts.Helvetica)
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold)
  const indigo = pdfColor(template.colors.primary)
  const slate = pdfColor(template.colors.text)
  const muted = pdfColor(template.colors.muted)
  const margin = template.spacing.margin

  page.drawRectangle({ x: 0, y: 712, width: template.page.width, height: template.spacing.headerHeight, color: indigo })
  page.drawText(template.labels.product, { x: margin, y: 752, size: 22, font: bold, color: rgb(1, 1, 1) })
  page.drawText(template.labels.document, { x: 475, y: 752, size: 15, font: bold, color: rgb(1, 1, 1) })
  page.drawText(organization.name, { x: margin, y: 726, size: 10, font: regular, color: rgb(1, 1, 1) })
  ...
  page.drawRectangle({ x: margin, y: 185, width: 528, height: 84, color: pdfColor(template.colors.summary) })
  ...
  page.drawText(template.labels.footer, { x: margin, y: 72, size: 8, font: regular, color: muted })
